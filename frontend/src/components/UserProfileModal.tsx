  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end">
      ...
      <motion.div
        ...
        className="relative w-full max-w-md h-full bg-dark-card border-l border-dark-border overflow-y-auto shadow-2xl flex flex-col"
      >
        {/* Botón cerrar flotante sobre el banner */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 p-1.5 rounded-xl bg-black/40 hover:bg-black/60 backdrop-blur-sm text-white border border-white/10 transition-all"
        >
          <X size={16} />
        </button>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="shrink-0 p-0">
              ...
              <div className="px-5 pb-4 bg-dark-card">
                ...
                {/* Bio corta */}
                {profile.bio && (
                  <p className="text-xs text-dark-muted italic mt-2 line-clamp-2 border-l-2 border-dark-border pl-2.5">
                    {profile.bio}
                  </p>
                )}
              </div>
                  ...
                  <div className="bg-dark-bg rounded-2xl border border-dark-border p-4 space-y-3">
                    ...
                    {profile.proyectos.slice(0, 5).map((p: any) => (
                      <div key={p.id} className="flex items-center gap-3 p-2.5 bg-dark-card/60 rounded-xl border border-dark-border/50">
                        <div className={`w-2 h-2 rounded-full shrink-0 ${p.estado === 'activo' ? 'bg-emerald-500' :
